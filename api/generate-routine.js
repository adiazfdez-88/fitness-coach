...
export default async function handler(req) {
  ...
  let profile, day, allDays, assignedGroup, usedExercises, isLastDay, lastWeekSummary;
  try {
    ({ profile, day, allDays, assignedGroup, usedExercises, isLastDay, lastWeekSummary } = await req.json());
  } catch {
  ...
  try {
    ...
    if (!plan) {
      return new Response(JSON.stringify({ error: 'La IA no devolvió un formato válido. Inténtalo de nuevo.' }), {
        status: 500, headers: { ...CORS, 'Content-Type': 'application/json' },
      });
    }
